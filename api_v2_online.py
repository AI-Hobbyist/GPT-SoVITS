...
```json
{
    ...
    "ref_audio_base64": "",       # str.(optional) reference audio base64 string
...
def b64_audio_to_file(b64_audio):
    ...
    if not Path("./refs").exists():
        Path("./refs").mkdir(parents=True, exist_ok=True)
    if not Path(f"./refs/{file_name}.wav").exists():
        audio_bytes = b64decode(b64_audio)
        Path(f"./refs/{file_name}.wav").write_bytes(audio_bytes)
        ref_audio_path = f"./refs/{file_name}.wav"
    else:
        ref_audio_path = f"./refs/{file_name}.wav"
...
def check_params(req:dict):
    ...
    ref_audio_base64:str = req.get("ref_audio_base64", "")
    ...
    if ref_audio_path in [None, ""] and ref_audio_base64 in [None, ""]:
        return JSONResponse(status_code=400, content={"message": "ref_audio_path or ref_audio_base64 is required"})
